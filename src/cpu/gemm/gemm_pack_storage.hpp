...
struct gemm_pack_storage_t {
    ...
    template <typename data_type>
    data_type *row_sums(int ithr, gemm_dim_t r0, gemm_dim_t cblock) const {
    ...
    template <typename data_type>
    data_type *col_sums(int ithr, gemm_dim_t rblock, gemm_dim_t c0) const {
    ...
    template <typename data_type>
    data_type *matrix(int ithr, gemm_dim_t r0, gemm_dim_t c0) const {
        assert(ithr < nthr());
    ...
    bool get_nocopy(int ithr, gemm_dim_t &ld, gemm_dim_t &td) const {
        assert(ithr < nthr());
        return matrix_header->slice[ithr].get_nocopy(ld, td);
    }

    bool get_nocopy(gemm_dim_t &ld, gemm_dim_t &td) const {
        if (!single_nocopy()) return false;
        return get_nocopy(0, ld, td);
    }

    void get_blocking(int ithr, gemm_dim_t &block_r, gemm_dim_t &block_c) const {
        assert(ithr < nthr());
        matrix_header->slice[ithr].get_blocking(block_r, block_c);
    }

    void set_blocking(int ithr, gemm_dim_t rows, gemm_dim_t cols, gemm_dim_t block_r,
            gemm_dim_t block_c) {
        assert(ithr < nthr());
    ...
    void set_nocopy(int ithr, gemm_dim_t ld, gemm_dim_t td) {
        assert(ithr < nthr());
...
protected:
    ...
    struct slice_header_t {
        ...
        template <typename data_type>
        size_t block_offset(gemm_dim_t r0, gemm_dim_t c0, bool col_major) const {
            assert((r0 % block_r) == 0);
            assert((c0 % block_c) == 0);
        ...
        void set_blocking(int nblk_r_, int nblk_c_, gemm_dim_t block_r_,
                gemm_dim_t block_c_) {
            packed = true;
            nblk_r = nblk_r_;
            nblk_c = nblk_c_;
        ...
        void set_nocopy(gemm_dim_t ld, gemm_dim_t td) {
            packed = false;
            block_r = ld;
            block_c = td;
            nblk_r = 1;
        ...
        void get_blocking(gemm_dim_t &block_r_, gemm_dim_t &block_c_) const {
            block_r_ = block_r;
            block_c_ = block_c;
        }
    ...
    template <typename data_type>
    data_type *get_block(const slice_header_t &slice, gemm_dim_t r0, gemm_dim_t c0) const
    {
