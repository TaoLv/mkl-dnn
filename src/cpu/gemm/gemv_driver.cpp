...
static void gemv_n_kernel(const gemm_dim_t m, const gemm_dim_t n, float alpha,
        const a_t * __restrict a, const gemm_dim_t lda, const b_t * __restrict x,
        const gemm_dim_t inc, c_t * __restrict y,
        const gemm_info_t<a_t, b_t, c_t> *arg) {
    if (inc == 1) {
        for (gemm_dim_t i = 0; i < n; i++) {
            for (gemm_dim_t j = 0; j < m; j++) {
                y[j] += alpha * x[i] * a[j + i * lda];
            }
        }
    } else {
        gemm_dim_t idx = inc < 0 ? (1 - n) * inc : 0;
        for (gemm_dim_t i = 0; i < n; i++) {
            for (gemm_dim_t j = 0; j < m; j++) {
...
template <typename a_t, typename b_t, typename c_t>
static void gemv_t_kernel(const gemm_dim_t m, const gemm_dim_t n, float alpha,
        const a_t * __restrict a, const gemm_dim_t lda, const b_t * __restrict x,
        const gemm_dim_t incy, c_t * __restrict y,
        const gemm_info_t<a_t, b_t, c_t> *arg) {
    ...
    if (mayiuse(sse41)) {
        ...
        if (incy == 1) {
            ...
            gemm_dim_t idy = incy < 0 ? (1 - n) * incy : 0;
            for (gemm_dim_t i = 0; i < n; i++) {
                c_t temp = (c_t) 0;
                for (gemm_dim_t j = 0; j < m; j++) {
                    temp += x[j] * a[j + i * lda];
...
static inline void gemv_kernel_driver(const int trans,
        const gemm_dim_t m, const gemm_dim_t n, const float alpha,
        const a_t *a, const gemm_dim_t lda, const b_t *x, const gemm_dim_t incx,
        const float beta, c_t *y, const gemm_dim_t incy,
        const gemm_info_t<a_t, b_t, c_t> *arg) {
    // Quick exit.
    ...
    gemm_dim_t idx_x = incx < 0 ? (1 - x_dim) * incx : 0;
    gemm_dim_t idx_y = incy < 0 ? (1 - y_dim) * incy : 0;

    // Scale the Y vector
    if (beta != 1.0f) {
        if (incy == 1) {
            if (beta == 0.0f) {
                for (gemm_dim_t i = 0; i < y_dim; i++) {
                    y[i] = (c_t) 0.0f;
                }
            } else {
                for (gemm_dim_t i = 0; i < y_dim; i++) {
            ...
            if (beta == 0.0f) {
                for (gemm_dim_t i = 0, inc = idx_y; i < y_dim; i++) {
                    y[inc] = (c_t) 0.0f;
                    inc += incy;
                }
            } else {
    ...
    if (trans == no_trans) { // A is not transpose.
        if (incy == 1) {
            ...
            if (!ytmp) {
                for (gemm_dim_t j = 0; j < n; j++) {
                    for (gemm_dim_t i = 0, inc = idx_y; i < m; i++) {
            ...
            gemm_dim_t m_blk = 0;
            for (gemm_dim_t i = 0; i < m; i += m_blk) {
                m_blk = m - i;
                if (m_blk > M_BLK)
                    m_blk = M_BLK;

                // Copy a block of y vector to temporary buffer.
                for (gemm_dim_t j = 0, inc = idx_y; j < m_blk; j++) {
                    ytmp[j] = y[inc];
                    inc += incy;
                }
        ...
        if (incx == 1) {
            ...
            if (!xtmp) {
                for (gemm_dim_t j = 0; j < n; j++) {
                    c_t acc = (c_t) 0.0f;
                    for (gemm_dim_t i = 0, inc = idx_x; i < m; i++) {
                        acc += x[inc] * a[i + j * lda];
                        inc += incx;
                    }
                    y[idx_y] += acc * alpha;
            ...
            for (gemm_dim_t i = 0; i < m; i += m_blk) {
                ...
                for (gemm_dim_t j = 0, inc = idx_x; j < m_blk; j++) {
                    xtmp[j] = x[inc];
                    inc += incx;
                }
...
static inline gemm_dim_t thread_checker(const gemm_dim_t m, const gemm_dim_t n) {
    ...
    gemm_dim_t bandt = n / nthr; // size per thread.
    ...
    if(nthr <= 12 && bandt < BAND_MIN) {
        ...
        if (bandt == 0) {
            ...
            return nstl::min(
                    gemm_dim_t(nstl::max((n * m) / gemm_dim_t(2 * MN_MIN_N), gemm_dim_t(1))),
                    nthr);
        }
    }
...
template <typename T>
static inline void decompose_vector(const gemm_dim_t m, const gemm_dim_t nthr,
        const gemm_dim_t ithr, T *addr, gemm_dim_t *offset, gemm_dim_t *size) {
    gemm_dim_t loffset = 0;
    gemm_dim_t lsize = 0;

    if (addr == NULL) {
        gemm_dim_t xthr  = m % nthr;
        gemm_dim_t width = m / nthr;
...
static inline void gemv_threading_driver(const int trans,
    ...
    gemm_dim_t nthr = thread_checker(m, n);
    ...
    // Execute in parallel mode
    parallel_nd((gemm_dim_t) nthr, [&](const gemm_dim_t ithr) {
        gemm_dim_t band, disp;
        decompose_vector(n, nthr, ithr, (c_t *) NULL, &disp, &band);

        gemm_dim_t ydisp = disp * incy;
        if (incy < 0)
