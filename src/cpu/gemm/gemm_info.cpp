...
void gemm_info_t<a_type, b_type, c_type>::jit_init(void) {
    ...
    // copyA[trans][sum]
    static void (*copyA[2][2])(const gemm_dim_t *m, const gemm_dim_t *n,
            const a_type *src, const gemm_dim_t *ldsrc, const float *alpha,
            a_type *dst, const gemm_dim_t *dummy1, const gemm_dim_t *dummy2,
            c_type *row_col_sum) = {{NULL}};

    // copyB[trans][sum]
    static void (*copyB[2][2])(const gemm_dim_t *m, const gemm_dim_t *n,
            const b_type *src, const gemm_dim_t *ldsrc, const float *alpha,
            b_type *dst, const gemm_dim_t *dummy1, const gemm_dim_t *dummy2,
            c_type *row_col_sum) = {{NULL}};

    // kern[beta0][col_off][row_off]
    static void (*kern[2][2][2])(const gemm_dim_t *m, const gemm_dim_t *n, const gemm_dim_t *k,
            const float *alpha, const a_type *a, const b_type *b, c_type *c,
            const gemm_dim_t ldc, const c_type *col_offset,
            const c_type *row_offset) = {{{NULL}}};

    // gemv_kern[trans]
    static void (*gemv_kern[2])(const gemm_dim_t *m, const gemm_dim_t *n,
            const float *alpha, const a_type*a, const gemm_dim_t *lda,
            const b_type *x, const gemm_dim_t *incy, c_type *y) = {NULL};

    static void (*gemv_s8u8s32_kern)(const gemm_dim_t, const gemm_dim_t, const float,
            const int8_t *, const gemm_dim_t, const uint8_t *, const float,
            int32_t *) = {NULL};

    static void (*gemv_u8s8s32_kern)(const gemm_dim_t, const gemm_dim_t, const float,
            const uint8_t *, const gemm_dim_t, const int8_t *, const float,
            int32_t *) = {NULL};
    ...
    std::call_once(initialized, []{
            ...
            for (int isSum : {no_sum, do_sum}) {
                ...
                if (p_copy_a != NULL)
                    copyA[isTrans][isSum] = p_copy_a->getCode<
                        void (*)(const gemm_dim_t *, const gemm_dim_t *, const a_type *,
                                const gemm_dim_t *, const float *, a_type *,
                                const gemm_dim_t *, const gemm_dim_t *, c_type *)>();
                auto *p_copy_b = copy_b[isTrans][isSum];
                if (p_copy_b != NULL)
                    copyB[isTrans][isSum] = p_copy_b->getCode<
                        void (*)(const gemm_dim_t *, const gemm_dim_t *, const b_type *,
                                const gemm_dim_t *, const float *, b_type *,
                                const gemm_dim_t *, const gemm_dim_t *, c_type *)>();
            }
                ...
                for (int isRowOffset : {no_row_offset, do_row_offset}) {
                    ...
                    if (p_kernel != NULL)
                        kern[isBeta0][isColOffset][isRowOffset] =
                            p_kernel->getCode<
                            void (*)(const gemm_dim_t *, const gemm_dim_t *,
                                    const gemm_dim_t *, const float *,
                                    const a_type *, const b_type *, c_type *,
                                    const gemm_dim_t, const c_type *,
                                    const c_type *)>();
                }
        ...
        if (data_traits<a_type>::data_type == data_type::f32) {
            for (int isTrans : {no_trans, do_trans}) {
                ...
                void (*)(const gemm_dim_t *, const gemm_dim_t *, const float *,
                        const a_type *, const gemm_dim_t *, const b_type *,
                        const gemm_dim_t *, c_type *)>();
