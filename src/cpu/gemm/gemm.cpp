...
mkldnn_status_t gemm_bf16bf16f32(const char *transa, const char *transb,
        ...
        const float *alpha,
        const bfloat16_t *A, const int *lda,
        const bfloat16_t *B, const int *ldb,
        const float *beta, float *C, const int *ldc) {
    int M_s32 = (int)*M;
    int N_s32 = (int)*N;
    int K_s32 = (int)*K;
...
mkldnn_status_t mkldnn_gemm_s8u8s32(const char *transa, const char *transb,
        ...
        int32_t *C, const int *ldc, const int32_t *co) {
    return gemm_s8x8s32(
        transa, transb, offsetc, M, N, K, alpha, A, lda, ao, B, ldb,
        (const uint8_t *)bo,
        beta, C, ldc, co);
}
...
mkldnn_status_t mkldnn_gemm_bf16bf16f32(const char *transa, const char *transb,
    ...
    return gemm_bf16bf16f32(transa, transb, M, N, K,
            alpha, (const bfloat16_t*)A, lda, (const bfloat16_t *)B, ldb,
            beta, C, ldc);
}
